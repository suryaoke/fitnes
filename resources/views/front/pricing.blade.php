...
@section('content')
    ...
    <main id="content"
        ...
        <img src="{{ asset('fitnes/assets/images/backgrounds/Illustration BG.svg') }}"
            class="absolute w-full h-full object-cover" alt="background">
                ...
                @forelse ($subscribePackages as $itemSubscribe)
                    <div class="card flex flex-col w-[356px] rounded-3xl p-8 gap-6 bg-white">
                        ...
                        <div class="flex flex-col gap-2">
                            <p class="font-['ClashDisplay-SemiBold'] leading-19 tracking-05"> {{ $itemSubscribe->name }}
                            </p>
                            <p class="text-sm leading-16 tracking-05 opacity-50">Enjoy all subscribe package benefits</p>
                        </div>

                        @foreach ($itemSubscribe->subcribeBenefits as $itemBenefit)
                            <div class="flex items-center gap-4">
                                <img src="{{ asset('fitnes/assets/images/icons/tick-circle.svg') }}"
                                    class="w-8 h-8 flex shrink-0" alt="icon">
                                <p class="leading-19 tracking-05">
                                    {{ $itemBenefit->name }}
                                </p>
                            </div>
                        @endforeach
                        ...
                        <div class="flex items-center justify-between mt-auto">
                            ...
                            <p class="text-right font-semibold leading-19 tracking-05">
                                Rp {{ number_format($itemSubscribe->price, 0, ',', '.') }} <span
                                    class="font-normal opacity-50">/<br>{{ $itemSubscribe->duration }}/days </span>
                            </p>
                        </div>
                    </div>
                @empty
