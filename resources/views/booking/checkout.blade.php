...
@section('content')
    ...
    <x-nav />
    <form action="{{ route('front.booking_store', $subscribePackage->id) }}" method="POST" id="content"
        ...
        <div class="flex flex-col gap-6 w-full max-w-[820px] shrink-0">
            <div id="account" class="flex flex-col w-full rounded-3xl p-8 gap-6 bg-white">
                <div class="flex flex-col gap-2">
                    ...
                    <p class="text-sm leading-16 tracking-03 opacity-60">Fill your data and make sure your contact before
                        checkout</p>
                </div>
                ...
                <label class="group flex items-center">
                    <p class="flex w-[162px] shrink-0 font-['ClashDisplay-SemiBold'] leading-19 tracking-05">Phone Number
                    </p>
            ...
            <div id="booking-items" class="flex flex-col w-full rounded-3xl p-8 gap-6 bg-white">
                <div class="flex flex-col gap-2">
                    ...
                    <p class="text-sm leading-16 tracking-03 opacity-60">Your next workout awaits, check booking details
                        here</p>
                </div>
                <hr class="border-black opacity-10">
                <div class="items flex flex-nowrap gap-4 w-full">
                    <img src="{{ asset('fitnes/assets/images/icons/cart.svg') }}" class="w-10 h-10 flex shrink-0"
                        alt="icon">
                    <div class="flex flex-col gap-2 w-full">
                        <div class="flex justify-between">
                            ...
                            <div class="flex gap-4 items-center">
                                <p class="font-['ClashDisplay-SemiBold'] leading-19 tracking-05"> Rp
                                    {{ number_format($subscribePackage->price, 0, ',', '.') }}</p>
                                <button class="appearance-none">
                                    <img src="{{ asset('fitnes/assets/images/icons/trash.svg') }}"
                                        class="w-4 h-4 flex shrink-0" alt="icon">
                                </button>
                            </div>
                        </div>
                        <p class="text-sm leading-16 tracking-03 opacity-60"> {{ $subscribePackage->duration }} / days - All
                            Benefits Included</p>
                    </div>
                </div>
                <hr class="border-black opacity-10">
            ...
            <div class="flex w-full aspect-[820/209]">
                <img src="{{ asset('fitnes/assets/images/thumbnails/banner.png') }}" class="w-full h-full object-contain"
                    alt="banner">
            </div>
        </div>
            ...
            <div class="flex flex-col w-full rounded-3xl p-8 gap-6 bg-white">
                ...
                <button type="button" class="w-full flex justify-between items-center rounded-lg p-4 bg-[#D0EEFF]">
                    <div class="flex items-center gap-3">
                        <img src="{{ asset('fitnes/assets/images/icons/ticket-discount.svg') }}"
                            class="w-8 h-8 flex shrink-0" alt="icon">
                        <p class="font-semibold leading-19 tracking-05">Use Promo Code</p>
                    </div>
                    <img src="{{ asset('fitnes/assets/images/icons/Vector.svg') }}" class="w-5 h-5 flex shrink-0"
                        alt="icon">
                </button>
            </div>
        </div>
